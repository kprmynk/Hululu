...
after bot get started use 
/genkey 99 days 

then redeem that key by 
/redeem key

then /bgmi ip port time 
/start to start attack
/stop to stop attack

- `/bgmi <target> <port> <time>` - SET FLOOD PERAMTER
- `/start` - After setting flood click start to start flooding to start attack.
- `/stop` - to stop the flood instant.
- to start agian again direct click on start then to stop click on stop
- for next game repeat 
...
For any queries or support, contact [BeasTxt_Sasuke](https://t.me/StoRm_BGMI_HACKS_STORE) on Telegram.
